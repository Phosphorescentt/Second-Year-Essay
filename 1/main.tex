...
\begin{definition}\label{def:open path}
  Let $N = (V, E)$ be a network, we say that a path in $N$ is \textbf{open} if:
  \begin{itemize}
...
\begin{definition}\label{def:openly disconnected}
  ...
  $B$. Throughout this essay, I will intercahnge the term \textbf{openly disconnected} with \textbf{disconnected}.\footnote{Definitions \ref{def:open path}, \ref{def:openly connected} and \ref{def:openly disconnected} are equivalent for both site and bond percolation.}
\end{definition}
